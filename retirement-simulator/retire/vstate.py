...
class VState:
    ...
    # Deferred tax carry-forward. The "tax-payment withdrawal" pass at the
    # end of each year sells taxable lots / pulls Trad / pulls Roth to fund
    # the year's tax bill. The gains/ordinary-income generated by those
    # sales properly belong on next year's return (you sold the stock in
    # April year+1 to settle year's tax; the gain is realized in year+1).
    # Each step function reads + zeros these at entry and writes new
    # deferred values at exit.
    deferred_lt_gain_n: np.ndarray = field(  # (P,) realized LT gains pending
        default_factory=lambda: np.zeros(0))
    deferred_st_gain_n: np.ndarray = field(  # (P,) realized ST gains pending
        default_factory=lambda: np.zeros(0))
